...
function UnselectAllSequences(seqs) {
  for (let otherSeq of seqs) {
    otherSeq.selected = false;
  }
}

function SelectFirstSequence(seqs) {
  UnselectAllSequences(seqs);
  seqs[0].selected = true;
}
...
function mousePressed() {
  for (let seq of sequences) {
    if (rectIntersect(seq)) {
      ...
      UnselectAllSequences(sequences)
